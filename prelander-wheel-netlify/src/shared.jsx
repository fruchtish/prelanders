...
const TrustBadges = () => (
  <div className="flex flex-wrap items-center gap-3 text-xs text-gray-700 font-semibold">
    ...
    <a
      className="px-2 py-1 rounded-full bg-gray-100 border hover:bg-gray-50"
  ...
  <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
    <div className="flex items-center gap-3">
      ...
      <div className="font-black">
        LuckyArcade<span className="text-rose-600">.io</span>
      </div>
    </div>
    <TrustBadges />
  </div>
...
const CTA = ({ children, ...rest }) => (
  <button
    {...rest}
    className="px-7 py-3.5 rounded-2xl shadow-xl text-white font-extrabold uppercase tracking-wider bg-gradient-to-r from-rose-600 to-amber-500 hover:from-rose-700 hover:to-amber-600"
  >
    {children}
  </button>
);

const OutlineCTA = ({ children, ...rest }) => (
  <button
    {...rest}
    className="px-5 py-2.5 rounded-xl border font-bold uppercase tracking-wide hover:bg-gray-50"
  >
    {children}
  </button>
);

const Countdown = ({ minutes = 10 }) => {
  ...
  const pct = Math.max(0, t / (minutes * 60)) * 100;

  return (
    <div className="w-full">
      <div className="text-sm font-bold mb-1 uppercase">
        Offer Expires In: <span className="text-rose-600">{mm}:{ss}</span>
      </div>
